...
class DualBlock(nn.Module):
    def __init__(self, features: int, kernel_size: Union[List[int], Tuple[int, int], int], complex=False, complex_weights=True):
        super().__init__()
        self.layers = nn.Sequential(
            PrePadDualConv2D(features+2, 32, kernel_size, complex=complex, complex_weights=complex_weights),
            cnn.CReLU() if complex else nn.PReLU(32),
            PrePadDualConv2D(32, 32, kernel_size, complex=complex, complex_weights=complex_weights),
            cnn.CReLU() if complex else nn.PReLU(32),
            PrePadDualConv2D(32, features, kernel_size, complex=complex, complex_weights=complex_weights),
        )
...
class PrimalBlock(nn.Module):
    def __init__(self, features: int, kernel_size: Union[List[int], Tuple[int, int], int], complex=False, complex_weights=True):
        super().__init__()
        self.layers = nn.Sequential(
            PrePadPrimalConv2D(features+1, 32, kernel_size, complex=complex, complex_weights=complex_weights),
            cnn.CReLU() if complex else nn.PReLU(32),
            PrePadPrimalConv2D(32, 32, kernel_size, complex=complex, complex_weights=complex_weights),
            cnn.CReLU() if complex else nn.PReLU(32),
            PrePadPrimalConv2D(32, features, kernel_size, complex=complex, complex_weights=complex_weights),
        )
        if complex:
...
class PrimalDualNetwork(nn.Module):
    def __init__(self,
        ...
        self.dual_blocks = nn.ModuleList([
            DualBlock(n_dual, complex=True if transform == "Fourier" else False, kernel_size=kernel_size, complex_weights=complex_weights) for _ in range(n_iterations)
        ])
