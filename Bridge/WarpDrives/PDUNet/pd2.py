...
class DualBlock(nn.Module):
    def __init__(self, features: int, complex=False, complex_weights=True, residuals=True):
        super().__init__()
    ...
    def forward(self, h: torch.Tensor, f: torch.Tensor, g: torch.Tensor):
        ...
        if self.residuals:
            ...
            return self.diff_weight.repeat(B, 1, H, W)*self.layers(block_input)
...
class PrimalUnetBlock(nn.Module):
    def __init__(self, features: int, complex=False, complex_weights=False, residuals=True):
        super().__init__()
    ...
    def __init__(self,
        ...
        self.primal_blocks = nn.ModuleList([
            (PrimalBlock if use_original_block else PrimalUnetBlock)(
                n_primary, complex=True if use_complex_primal and transform == "Fourier" else False, complex_weights=complex_weights, residuals=residuals)
            for _ in range(n_iterations)
        ])
        self.dual_blocks = nn.ModuleList([
            DualBlock(n_dual, complex=True if transform == "Fourier" else False, complex_weights=complex_weights, residuals=residuals) for _ in range(n_iterations)
        ])
