...
def getARGSParser():
    ...
    parser.add_argument('--trainID', action="store", default="ResNet5_fullVol_L1Loss_alemotion") ## "testing")  ## "ResNet14"
    ...
    #Training params
    parser.add_argument('--num_epochs', action="store", default=150, type=int, help="Total number of epochs. If resuming, then it will continue till a total number of epochs set by this.")
    ...
    parser.add_argument('--input_shape', action="store", default="256,256,150", help="length, width, depth (to be used if patch_size is not given)")
    ...
    #Augmentations
    parser.add_argument('--contrast_augment', action="store", default=1, type=int, help="Whether to user contrast augmentations")
    ...
    parser.add_argument('--model_upinterp_algo', action="store", default="trilinear", help='"convtrans", or interpolation technique: "sinc", "nearest", "linear", "bilinear", "bicubic", "trilinear", "area"')
    ...
    #Motion parameters, currently only used for TorchIO RandomMotionGhosting
    parser.add_argument('--motion_degrees', action="store", default="1.0,7.0", help="Tuple of Float, passed as CSV")
    parser.add_argument('--motion_translation', action="store", default="1.0,7.0", help="Tuple of Float, passed as CSV")
    parser.add_argument('--motion_num_transforms', action="store", default="3,10", help="Tuple of Int, passed as CSV")
    parser.add_argument('--motion_num_ghosts', action="store", default="3,10", help="Tuple of Int, passed as CSV")
    parser.add_argument('--motion_intensity', action="store", default="0.01,1.0", help="Tuple of Float, passed as CSV")
    parser.add_argument('--motion_restore', action="store", default="0.01,1.0", help="Tuple of Float, passed as CSV")
    parser.add_argument('--motion_image_interpolation', action="store", default='linear')
    parser.add_argument('--motion_ghosting_axes', action="store", default="0,1", help="Tuple of Int, passed as CSV")
    parser.add_argument('--motion_p_motion', action="store", default=0.75, type=float)
    parser.add_argument('--motion_p_ghosting', action="store", default=0.75, type=float)
