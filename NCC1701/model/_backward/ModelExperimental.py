...
class Model(object):
    ...
    def __init__(self, domain, batchSize = 1, imageSize = None, type = '2DMultislice', pin_memory = False, IsRadial = False, undersamplingMask = None, log_path = None):
        ...
        if undersamplingMask is not None:
            self.undersamplingMask = torch.from_numpy(undersamplingMask)
            self.missingMask = torch.from_numpy((~undersamplingMask.astype(bool)).astype(int))
        else:
    ...
    def ToCUDA(self, IsMultiGPU = True):
        ...
        self.net_img = self.net_img.cuda().contiguous()
    ...
    def ToCPU(self):
        ...
        self.net_img = self.net_img.cpu().contiguous()
    ...
    def CreateModel(self, netModel, netModel4ksp, IsCuda = False, IsMultiGPU = False, n_channels = 1, initialLearningRate = 0.001, lrDecayNEpoch = 4, lrDecayRate = 0.5, betas = (0.9, 0.999), epsilon = 1e-08, loss_func = nn.MSELoss, optimizer=optim.Adam):
        ...
        if inspect.isclass(netModel):# We create the generator object
            self.net_img = netModel(n_channels)
        else:
            self.net_img = netModel 
        self.net_img.apply(self.InitializeWeights) # We initialize all the weights of its neural network.

        if inspect.isclass(netModel4ksp):# We create the generator object
            self.net_ksp = netModel4ksp(n_channels)
        else:
            self.net_ksp = netModel4ksp 
        ...
        if(self.IsCuda):
            ...
            self.criterion.cuda()
            if(self.IsMultiGPU):
                self.net_img = nn.DataParallel(self.net_img) #This makes use of all the GPUs.  
                self.net_ksp = nn.DataParallel(self.net_ksp) #This makes use of all the GPUs.  
      
        #Adam optimizer
        #net.parameters() - Paramters of our NN
        #lr - Learning Rate
        #betas - coefficients used for computing running averages of gradient and its square
        self.optimizer = optimizer(self.net_img.parameters(), lr = initialLearningRate, betas = betas, eps = epsilon) # We create the optimizer object of the generator.
    ...
    def Train(self, dataloader = None, total_n_epoch = 25, start_epoch = 0, root_path = '', save_frequency = 10):
        ...
        for epoch in range(start_epoch, total_n_epoch): # We iterate over n_epoch epochs.             
            ...
            self.helpOBJ.save_checkpoint({
                'epoch': epoch + 1,
                'model': self.net_img,
                'state_dict': self.net_img.state_dict(),
    ...
    def TrainOneEpoch(self, dataloader, epoch, n_epoch):
        ...
        for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (training).
            ...
            fully = data['fully']
            #under = data['under']        
            fully_k = self.helpOBJ.torchFFT(fully)
            under_k = fully_k
            under_k[0,0,:,:] = fully_k[0,0,:,:]*self.underMask
            under_k[0,1,:,:] = fully_k[0,1,:,:]*self.underMask
            #only supports 2DSingleSlice and batch size 1
            ...
            fully = Variable(fully_k)
            under = Variable(under_k)
            if(self.IsCuda):
                ...
                fully = fully.cuda()
                under = under.cuda()
            generated = self.net_img(under) # We forward propagate the undersampled images into the neural network of the generator to get some "improved" images.
    ...
    def Validate(self, dataloader, saveOutput=False, print_freq=10, epoch=None, n_epoch=None, output_path=None, IsTest=False):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (testing/prediction).
                ...
                under = self.helpOBJ.torchIFFT(under_k)
                generated = self.helpOBJ.torchIFFT(generated)
