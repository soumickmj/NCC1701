...
class Model(object):
    ...
    def IntitializeMRITorchDS(self, dsClass, sliceno, startingSelectSlice, endingSelectSlice, folder_path_fully, folder_path_under, extension_under, num_workers, getROIMode):
    ...
    def CreateModel(self, netModel, IsCuda = False, IsMultiGPU = False, n_channels = 1, initialLearningRate = 0.001, lrDecayNEpoch = 4, lrDecayRate = 0.5, betas = (0.9, 0.999), epsilon = 1e-08):
        ...
        self.net = netModel(n_channels) # We create the generator object.
        ...
        self.l1regul = nn.L1Loss(size_average=False)
    ...
    def TrainOneEpoch(self, dataloader, epoch, n_epoch, sliceno = 50):
        ...
        for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (training).
            ...
            #    reg_loss += self.l1regul(param, target=torch.zeros_like(param))
            
            #regulFactor = 0.0005
            #self.err += regulFactor * reg_loss

            l1_regularization, l2_regularization = torch.tensor(0).type(torch.cuda.FloatTensor), torch.tensor(0).type(torch.cuda.FloatTensor)
            for param in self.net.parameters():
                l1_regularization += torch.norm(param, 1)
                l2_regularization += torch.norm(param, 2)

            self.err += 0.5 * l1_regularization + 0.01 * l2_regularization
