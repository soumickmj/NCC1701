...
This work with patches. This approach can be used for Discriminator, but this can't be used in general.
...
class Model(object):
    ...
    def __init__(self, domain, batchSize = 1, imageSize = None, pin_memory = False):
    ...
    def IntitializeMRITorchDS(self,folder_path_fully, folder_path_under, extension_under, num_workers, getROIMode):
        ...
        transform = transforms.Compose([transformsMRI.ToTensor3D(),]) # We create a list of transformations (3dto2d, tensor conversion) to apply to the input images.
        
        # Loading the dataset
        dataset = MRITorchDS(folder_path_fully,folder_path_under,extension_under,domain=self.domain,transform=transform,getROIMode=getROIMode)
    ...
    def CreateConvTrans(self, IsCuda = False, IsMultiGPU = False, n_channels = 1, initialLearningRate = 0.001, lrDecayNEpoch = 4, lrDecayRate = 0.5, betas = (0.9, 0.999), epsilon = 1e-08):
        ...
        # Creating the ConvolutionalTranslator
        self.net = Translator(n_channels) # We create the generator object.
        self.net.apply(self.InitializeWeights) # We initialize all the weights of its neural network.
        self.criterion = nn.MSELoss() # MSE Loss -  We create a criterion object that will measure the error between the prediction and the target.
        ...
        self.optimizer = optim.Adam(self.net.parameters(), lr = initialLearningRate, betas = betas, eps = epsilon) # We create the optimizer object of the generator.
        self.initialLearningRate = initialLearningRate
        self.lrDecayNEpoch = lrDecayNEpoch
        self.lrDecayRate = lrDecayRate
                   
    def Train(self, dataloader = None, total_n_epoch = 25, start_epoch = 0, root_path = ''):
        ...
        for epoch in range(start_epoch, total_n_epoch): # We iterate over n_epoch epochs.
            self.TrainOneEpoch(dataloader, epoch, total_n_epoch) #Train for one epoch
    ...
    def TrainOneEpoch(self, dataloader, epoch, n_epoch):
        ...
        for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (training).
            ...
            #under = data['under']
            fully = data['fully']
            fully = fully.unfold(3, 128, 24).unfold(4, 128, 24)
            fully = fully.contiguous().view(fully.size(0),fully.size(1),fully.size(2)*fully.size(3)*fully.size(4),fully.size(5),fully.size(6))
            fully = fully.permute(1,0,2,3,4).view(fully.size(0),fully.size(1)*fully.size(2),fully.size(3),fully.size(4)).permute(1,0,2,3)
            under = data['under']
            under = under.unfold(3, 128, 24).unfold(4, 128, 24)
            under = under.contiguous().view(under.size(0),under.size(1),under.size(2)*under.size(3)*under.size(4),under.size(5),under.size(6))
            under = under.permute(1,0,2,3,4).view(under.size(0),under.size(1)*under.size(2),under.size(3),under.size(4)).permute(1,0,2,3)
    ...
    def Test(self, dataloader, output_path):
        ...
        self.Validate(dataloader, saveOutput=True, print_freq=10, epoch=None, n_epoch=None, output_path=output_path)
        
    def Validate(self, dataloader, saveOutput=False, print_freq=10, epoch=None, n_epoch=None, output_path=None):
        ...
        #Initialize Params
        transformTensorToNumpy2D = transformsMRI.FromTensorToNumpy2D()        
        batch_time = EvaluationParams()
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (testing/prediction).
                ...
                generated = generated.permute(1,0,2,3).unsqueeze(1)
                #generated = generated.view(generated.size(0),n_channels,generated.size(2)/n_channels,generated.size(2),generated.size(3))
                ...
                #acc = self.helpOBJ.accuracy(generated, fully)               # send param 2D or 3D to this function
                acc = [0]  
                ...
                if(saveOutput):    
                    ...
                    for j in range(0, len(data['subjectName'])): 
                        ...
                        generatedImg = self.helpOBJ.Domain2Image(transformTensorToNumpy2D(generated.data[j]))
                        fullyImg = self.helpOBJ.Domain2Image(transformTensorToNumpy2D(fully[j]))
                        underImg = self.helpOBJ.Domain2Image(transformTensorToNumpy2D(under[j]))
