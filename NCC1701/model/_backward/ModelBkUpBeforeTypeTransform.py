        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (testing/prediction).
                if(self.type == '2DSingleSlice'):
                    ...
                    under = data['under'][:,:,sliceno,:,:]
                    under = Variable(under)
                    under = under.contiguous().cuda(async=True)
                    generated = self.net(under)
                elif(self.type == '2DMultiSlice'):
                    fully = data['fully']
                    under = data['under']
                    under4net = under.permute(1,0,2,3,4)
                    under4net = under4net.view(under4net.size(0),under4net.size(1)*under4net.size(2),under4net.size(3),under4net.size(4)).permute(1,0,2,3)
                    under4net = Variable(under4net)
                    generated = self.net(under4net)
