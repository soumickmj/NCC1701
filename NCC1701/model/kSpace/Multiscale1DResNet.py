...
"""
Implimention of Multi Scale 1D ResNet for Inverse Problems.
...
1.  Standard ResNet models can be created using the creator functions. See below for the list of creator functions.
    ...
    Allowed params with these creator functions: in_channels, out_channel, n_scales, starting_nplanes, act, skipmode, interp4upconv,  downsamAtStart, dilation, n_fullScaleBlocks
...
def _upconv(in_planes, out_planes, kernel, stride=1, interpolation=None, groups=1, dilation=1):
    ...
    if interpolation is not None:
        ...
        return nn.ConvTranspose1d(in_planes, out_planes, kernel_size=kernel, stride=stride,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel, stride=1, downupsample=None, isup=False, act=nn.PReLU, interp4upconv=None, groups=1,
                 base_width=64, dilation=1, semifinalconv=False):
    ...
    def forward(self, x):
        ...
        if d == 0:
            ...
            d = abs(d)
            out = residual + F.pad(out, (d//2, d-(d//2)))
        else:
            out = residual + out[:,:,0:d]
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel, stride=1, downupsample=None, isup=False, act=nn.PReLU, interp4upconv=None, groups=1,
        ...
        self.bn3 = nn.BatchNorm1d(planes * expansion_local)
...
class MSResNet(nn.Module):
    def __init__(self, in_channels=1, out_channel=1, n_layers=3, n_scales=3, layers=1, useBottleneck=False, starting_nplanes=64, act=nn.PReLU, skipmode=0, interp4upconv=None, 
                downsamAtStart=True, groups=1, width_per_group=64, dilation=1, n_fullScaleBlocks=0):
        ...
        self.conv1 = nn.Conv1d(in_channels, starting_nplanes, kernel_size=7, stride=2 if downsamAtStart else 1, padding=3,
                               bias=False)
        self.bn1 = nn.BatchNorm1d(starting_nplanes)
        ...
        for i in range(n_scales):
            downlayer, uplayer, semifinallayer = self._make_layers(Block, layers, n_layers=n_layers, scale_index=i, stride=2)
            self.downlayers.append(downlayer)
            self.uplayers.append(uplayer)
            self.semifinalConvs.append(semifinallayer)

        self.convFC = nn.ConvTranspose1d(sum(self.multiscale_inplanes), out_channel, kernel_size=7, stride=2 if downsamAtStart else 1, padding=3, 
        ...
        for i in range(n_layers):
            ...
            if stride != 1 or (n_planes[0] if scale_index==-1 else self.multiscale_inplanes[scale_index]) != planes * block.expansion:
                downsample = nn.Sequential(
                    nn.Conv1d((n_planes[0] if scale_index==-1 else self.multiscale_inplanes[scale_index]), planes * block.expansion,
        ...
        for i in reversed(range(n_layers)):
            ...
            if stride != 1 or (n_planes[0] if scale_index==-1 else self.multiscale_inplanes[scale_index]) != planes * block.expansion:
                upsample = nn.Sequential(
                    nn.ConvTranspose1d((n_planes[0] if scale_index==-1 else self.multiscale_inplanes[scale_index]), planes * block.expansion,
            ...
            if self.skipmode==1:
                resi_connector = nn.Sequential(
                    nn.Conv1d(semifinal_inplanes, planes * block.expansion,
            ...
            semifinal_modules.append(block(semifinal_inplanes, planes* block.expansion, kernel, downupsample=resi_connector, isup=False, act=self.act, 
                groups=self.groups, base_width=self.base_width, dilation=self.dilation, semifinalconv=True))
