...
class CInvUNet(nn.Module):
    def __init__(self, n_channels, drop_prob=0.2, up_factor=2, complex_latent=False, latent_conv=False, latent_n_features=32):
        super(CInvUNet, self).__init__()

        if complex_latent:
            latent_size = latent_n_features*2
        else:
            latent_size = latent_n_features
