...
class ResNet(nn.Module):
    def __init__(self, n_channels=1, res_blocks=14, starting_nfeatures=1, updown_blocks=2, underDownStrategy='cropcenter', w_size=256, h_size=256, missingMask=None): #should use 14 as that gives number of trainable parameters close to number of possible pixel values in a image 256x256 
        ...
        self.missingMask = torch.from_numpy(self.imgFlatten(missingMask)).float().cuda(0)
    ...
    def forward(self, x):
        ...
        x = self.intialConv(under.cuda(1)) #256
        x = x * self.missingMask.cuda(1)
        x = x + under.cuda(1)

        x = self.downsam(x.cuda(2)) #64
        x = x * self.missingMaskDown.cuda(2)
        x = x + underDown.cuda(2)

        x = self.resblocks(x.cuda(3)) #64
        x = x * self.missingMaskDown.cuda(3)
        x = x + underDown.cuda(3)

        x = self.upsam(x.cuda(4)) #256
        x = x * self.missingMask.cuda(4)
        x = x + under.cuda(4)

        x = self.finalconv(x.cuda(5)) #256
        x = x * self.missingMask.cuda(5)
        x = x + under.cuda(5)

        x = self.imgUnFlatten(x.cuda(0))
