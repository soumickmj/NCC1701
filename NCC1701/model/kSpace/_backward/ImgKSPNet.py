...
class ImgKSPNet(nn.Module):
    ...
    def __init__(self, n_channels, underMask=None, input_size=256, ksp_net=BottleNeckNet, img_net=ResNet, ksp_bottle_neck_size=64, ksp_model_type='simplelinear', ksp_activation='prelu', ksp_normalization='instance', multilossreturn=False):
        super().__init__()

        self.multilossreturn = multilossreturn
    ...
    def forward(self, x):     
        ...
        out_img_net = self.img_net(under)
        out_img_net_k = rfft2c(out_img_net, False)

        #Operations in the kSpace
        if self.splitgpu:
            out_ksp_net_k = self.ksp_net(under_k.cuda(1)).cuda(0)
        else:
            out_ksp_net_k = self.ksp_net(under_k)

        #Combined operations
        out_combined_k = (out_img_net_k + out_ksp_net_k)/2
        missing_k = out_combined_k*self.missingMask
        final_k = missing_k + under_k
        final = irfft2c(final_k)

        if self.multilossreturn:
            #final (img), out_img_net (img), out_ksp_net_k (ksp), fully_k (ksp) [fully_k neeed for loss calculation]
            return self.finalactivation(final), self.finalactivation(out_img_net), out_ksp_net_k, fully_k
        else:
