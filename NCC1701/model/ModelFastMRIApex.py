...
class Model(object):
    ...
    def __init__(self, domain, batchSize = 1, imageSize = None, type = '2DMultislice', pin_memory = False, IsRadial = False, undersamplingMask = None, log_path = None, amp_opt_level=None):
    ...
    def IntitializeFastMRITorchDS(self, dsClass, folder_path, num_workers, transform=None, getROIRes=None, filename_filter = ['Aera','Biograph_mMR','Prisma_fit', 'Skyra'], 
        ...
        # Loading the dataset
        dataset = dsClass(folder_path, self.dsdomain, transform, getROIRes, filename_filter, ds_split_xlsx, corupt_type, mask_func, mask_seed, undersampling_mat, top_n_sampled)
        dataloader = DataLoader(dataset, batch_size = self.batchSize, shuffle = True, num_workers = num_workers, pin_memory = self.pin_memory)
    ...
    def criterion(self, generated, fully):
        if self.loss_type == 0:
            ...
            g = self.kSpace2ImgSpace(generated)
            f = self.kSpace2ImgSpace(fully)
            if self.loss_type == 1:
                return self.loss_func(g, f)
            else:
                return self.loss_func(g/g.max(), f/f.max())

    def kSpace2ImgSpace(self, data):
        if self.domain == 'fourier':
            data = transformsFastMRI.complex_abs(transformsFastMRI.ifft2(transformsFastMRI.channel_to_complex(data,True)))
        elif self.domain == 'hartley':
            data = transformsFastMRI.hartley_transform(data, no_centring=False) #TODO HOTFIX no_centring
        return data
    ...
    def Train(self, dataloader = None, total_n_epoch = 25, start_epoch = 0, root_path = '', save_frequency = 10):
        ...
        for epoch in range(start_epoch, total_n_epoch): # We iterate over n_epoch epochs.             
            ...
            self.helpOBJ.save_checkpoint({
                ...
                'amp': amp.state_dict() if self.amp_opt_level is not None else None
    ...
    def TrainOneEpoch(self, dataloader, epoch, n_epoch):
        ...
        for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (training).
            ...
            try:
                ...
                if self.amp_opt_level is not None:
                    with amp.scale_loss(self.err, self.optimizer) as scaled_loss:
    ...
    def PrepareImg4TB(self, tensor):
        C = tensor.shape[1]
        #data = self.kSpace2ImgSpace(tensor)[0,...].squeeze() #TODO: Fix this. Right now, only taking the first image of the batch.
        data = tensor[0,...].squeeze()
        if C == 1:
            ...
            for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (testing/prediction).
                ...
                try:
                    ...
                    if self.NetType == 1 or self.NetType == 3: #KSPNet4FastMRI or KSPNet
                        ...
                        generated, mu, logvar = self.net(under)
                        loss = self.net.loss_function(generated, fully, mu, logvar)
                    else:
                        generated = self.net(under) 
                    ...
                    if self.domain == 'fourier' or self.domain == 'hartley': #TODO> Hotfix
                        under = self.kSpace2ImgSpace(under)
                        generated = self.kSpace2ImgSpace(generated)
                        fully = self.kSpace2ImgSpace(fully)
                        convertedToDomain = True ####TODO Hotfix. Need to deicde using param
                    else:
                        convertedToDomain = False
                    ...
                    if i % print_freq == 0:
                        ...
                        self.tb_writer.add_images(tb_tag_type+'Fully', self.PrepareImg4TB(fully), niter, dataformats='NCHW')
                        self.tb_writer.add_images(tb_tag_type+'Under', self.PrepareImg4TB(under), niter, dataformats='NCHW')
                        self.tb_writer.add_images(tb_tag_type+'Recon', self.PrepareImg4TB(generated), niter, dataformats='NCHW')
                        runningLoss = 0
                        runningAcc = 0
                        runningLossCounter = 0
