...
Takes input as Image and then output also Image
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x_f = torch.rfft(x[0,0,:,:], signal_ndim=2, normalized=True, onesided=False)
        x_f.unsqueeze_(0).unsqueeze_(0)
        x_f = x_f.permute(0,1,4,2,3)
        x_f = x_f.view(x_f.size(0),x_f.size(1)*x_f.size(2),x_f.size(3),x_f.size(4))
        y_f = self.model(x_f)
