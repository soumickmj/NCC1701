...
class ResNet(nn.Module):
    def __init__(self, n_channels=1, res_blocks=10, starting_nfeatures=16, updown_blocks=2, is_relu_leaky=True, do_batchnorm=False, forwardV=0, upinterp_algo='trilinear'): #should use 14 as that gives number of trainable parameters close to number of possible pixel values in a image 256x256 
        super(ResNet, self).__init__()

        print('creating resnet3d')
        print(res_blocks)
        print(starting_nfeatures)
        print(updown_blocks)
        print(forwardV)
        ...
        for _ in range(updown_blocks):
            upsam.append(nn.Sequential(  
                        #ReflectionPad3d(1),
                        nn.Conv3d(in_features, out_features, 1),
        ...
        if forwardV == 6: #SuperResNet
            self.sliceup1 = nn.Sequential(  
                            nn.Conv3d(starting_nfeatures, starting_nfeatures, 1),
                            norm(starting_nfeatures),
                            relu())
    ...
    def forward(self, x, slthkns_src=1.5, slthkns_trgt=2):
        if self.forwardV == 0:
            #v0: Original Version
            x = self.intialConv(x)
            shapes = []
            ...
            for i, upblock in enumerate(self.upsam):
                outs[-1] = F.interpolate(outs[-1], shapes[-1-i], mode=self.upinterp_algo, align_corners=True)
