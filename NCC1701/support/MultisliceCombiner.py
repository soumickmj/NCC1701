...
def ListSubset(listOfOneSub):
    subDict = defaultdict(list)
    for item in listOfOneSub:
        subDict['_'.join(item.split('_')[:-1])].append(item)
    return subDict
...
def MultisliceCombiner(root_path_input, root_path_output, isFolderInFolder=True, isAnyCorrected=False):
    ...
    for folder in folders:   
        ...
        subDict = ListSubset(subfolders)
        for key in subDict.keys():
            subfolder_output = key
            subsubfolders = subDict[key]
            ...
            for subsubfolder in subsubfolders:
                fullpath_subfolder = os.path.join(fullpath_folder, subsubfolder)
