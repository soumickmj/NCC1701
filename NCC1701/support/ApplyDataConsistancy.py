...
def ApplyDataConsis(root_path, isRadial, doNorm, mask_or_om_path, interpolationSize4NUFFT = 6, usCGSolver = False, imageSize = 256, updateStats = True):
    ...
    for folder in folders:   
        ...
        for subfolder in subfolders:
            ...
            fully = FileRead2D(os.path.join(fullpath_subfolder, 'fully.nii')).squeeze()
            recon = FileRead2D(os.path.join(fullpath_subfolder, 'recon.nii')).squeeze()

            if(updateStats):
                under = FileRead2D(os.path.join(fullpath_subfolder, 'under.nii')).squeeze()
