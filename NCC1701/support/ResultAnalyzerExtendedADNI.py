...
def AnalyzeResultFolder(root, folder, adni_csv, acq_type='MPRAGE', isAnyCorrected=False, useExtended=True):
    ...
    if(isAnyCorrected):
        ...
        worksheet.write(row, col+20, 'ADNIGroup', title_format)   
    else:    
        ...
        worksheet.write(row, col+16, 'ADNIGroup', title_format)
    ...
    for folder in folders:  
        ...
        for subfolder in subfolders:
            scan = subfolder
            acqdate = scan.split('_')[0].split('-')
            acqdate = datetime.date(int(acqdate[0]), int(acqdate[1]), int(acqdate[2]))
            adni_groups = df['Group'].where(df['Subject'] == subject).where(df['Description'] == acq_type).where(df['Acq Date'] == acqdate).dropna().unique().tolist()
            ...
            adni_groups = ",".join(set(adni_groups))
            row += 1
