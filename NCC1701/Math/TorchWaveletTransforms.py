...
"""
This module deals in transform to and from wavelet domain.

Jobs:-
    1. Discrete Wavelet Transform (+Inverse) [2D]
    2. Stationary wavelet transform (or undecimated wavelet transform) (Inverse not yet implimented) [2D]
"""
...
from . import DWTSupport as lowlevel

__author__ = "Praveen Putti"
__copyright__ = "Copyright 2019, Soumick Chatterjee & OvGU:ESF:MEMoRIAL"
__credits__ = ["Praveen Putti", "Soumick Chatterjee"]
...
class DWTForward(nn.Module):
    ...
    Args:
        ...
        """
    def __init__(self, J=1, wave='db1', mode='zero', trainable=False):
        super().__init__()
        ...
        self.h0_col = nn.Parameter(filts[0], requires_grad=trainable)
        self.h1_col = nn.Parameter(filts[1], requires_grad=trainable)
        self.h0_row = nn.Parameter(filts[2], requires_grad=trainable)
        self.h1_row = nn.Parameter(filts[3], requires_grad=trainable)
        self.J = J
        self.mode = mode
...
class DWTInverse(nn.Module):
    ...
    """
    def __init__(self, wave='db1', mode='zero', trainable=False):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
        ...
        self.g0_col = nn.Parameter(filts[0], requires_grad=trainable)
        self.g1_col = nn.Parameter(filts[1], requires_grad=trainable)
        self.g0_row = nn.Parameter(filts[2], requires_grad=trainable)
        self.g1_row = nn.Parameter(filts[3], requires_grad=trainable)
        self.mode = mode
...
class SWTForward(nn.Module):
    ...
    Args:
        ...
        """
    def __init__(self, J=1, wave='db1', mode='periodization', trainable=False):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
