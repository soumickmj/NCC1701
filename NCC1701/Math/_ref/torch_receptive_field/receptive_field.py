"""
Original GitHub repo: https://github.com/Fangyh09/pytorch-receptive-field
Explaination: https://medium.com/mlreview/a-guide-to-receptive-field-arithmetic-for-convolutional-neural-networks-e0f514068807

Added for handling Padding Layers, InstanceNorm2d
"""
...
def receptive_field(model, input_size, batch_size=-1, consider_paddingclasses=False, device="cuda"):
    '''
    :parameter
    ...
    '''
    paddingLayersStack = []
    paddingClasses = tuple(x[1] for x in inspect.getmembers(torch.nn.modules.padding,inspect.isclass) if x[0] !='Module' )
    def register_hook(module):

        def hook(module, input, output):
            class_name = str(module.__class__).split(".")[-1].split("'")[0]

            if class_name == "Dropout2d": #TODO- not implimented
                return
            elif class_name == "ResidualBlock": #TODO- not implimented - DO IT NOW
                return
            elif isinstance(module, paddingClasses):
                paddingLayersStack.append(module.padding[::2])
                #it doesn't consider if left and right have different padding, or if top and bottom have different padding, or front and back have different padding
                #It will consider the first one for both, as convolution layers doesn't use seperate padding for them
                return
            ...
            if not receptive_field["0"]["conv_stage"]:
                ...
                if class_name == "Conv2d" or class_name == "MaxPool2d":
                    ...
                    if consider_paddingclasses:
                        while len(paddingLayersStack) > 0:
                            padding = tuple(np.add(padding, paddingLayersStack.pop()))
                ...
                elif class_name == "BatchNorm2d" or class_name == "InstanceNorm2d" or class_name == "ReLU" or class_name == "Bottleneck": #InstanceNorm2d added by Soumick
