...
maskORom_path = r"/mnt/BMMR/data/Soumick/DS6NCC1701/Hendrik/Under/2DVarden05Mask.mat" #Mark: Path to the mat file
import scipy.io as sio
mat_contents = sio.loadmat(maskORom_path)
try:
    mask = mat_contents['mask']
except:
    mask = np.zeros((256,256)) #change this
missingMask = (~mask.astype(bool)).astype(int)
...
IsRadial = False #Mark: set it to True for Radial

#placeholders, to be ignored in newer files
...
save_frequency = 1
IsResume = True
CheckpointPath = r"/mnt/BMMR/data/Soumick/DS6NCC1701/RoughOut/Results/ReproAttempt8-IXIT1HHVarden1D15-Resnet2Dv214PReLU-SSIMLoss-AllSlices/checkpoints/checkpoint.pth.tar"
...
#netModel = importlib.import_module('model.kSpace.ImgKSPNet').ImgKSPNet 
#from model.kSpace.ImgKSPNet import ImgKSPNet
#netModel = ImgKSPNet(n_channels, ksp_bottle_neck_size=128, ksp_model_type='simplelinear')
#IsImgKSPNet = True
...
tb_logroot = '/run/media/soumick/Enterprise/OutputNewPipe/TBLogs'
train_path_fully = r'/run/media/soumick/Enterprise/Datasets/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/train'
test_path_fully = r'/run/media/soumick/Enterprise/Datasets/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/test'
val_path_fully = r'/run/media/soumick/Enterprise/Datasets/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/val'
output_path = r'/run/media/soumick/Enterprise/OutputNewPipe/Results'
output_folder = 'ReproAttempt8old-IXIT1HHVarden1D15-Resnet2Dv214PReLU-SSIMLoss-AllSlices'
# output_folder = 'testing'
output_path = os.path.join(output_path, output_folder)
tb_logpath = os.path.join(tb_logroot, output_folder)

#can ignore these - just placeholders to avoid errors, not used in case of MATs
train_path_under = r'/mnt/BMMR/data/Soumick/DS6NCC1701/Hendrik/Under/2DVarden05Mask/train' 
train_entension_under = 'nii.gz'
val_path_under = r'/mnt/BMMR/data/Soumick/DS6NCC1701/Hendrik/Under/2DVarden05Mask/val'
test_entension_under = 'nii.gz'
test_path_under = r'/mnt/BMMR/data/Soumick/DS6NCC1701/Hendrik/Under/2DVarden05Mask/test'
val_entension_under = 'nii.gz'
...
#     train_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\Guys\Slice60to90\Varden1D15SameMask\train'
#     train_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TrainSet\varden30SameMask'
#     train_entension_under = 'nii.gz'
#     test_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\Guys\Slice60to90\Varden1D15SameMask\test'
#     test_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TestSet\varden30SameMask'
#     test_entension_under = 'nii.gz'
#     val_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\Guys\Slice60to90\Varden1D15SameMask\val'
#     val_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TestSet\varden30SameMask'
#     val_entension_under = 'nii.gz'
#     output_path = r'D:\Output\Gen3\IXI-T1\Guys\Slice60to90'
#     output_folder = 'Attempt2-IXIT1GuysVarden1D15-Resnet2Dv214PReLU-SSIMLoss-Slice60to90'
#     output_path = os.path.join(output_path, output_folder)
#     tb_logpath = os.path.join(tb_logroot, output_folder)
...
#     train_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/train'
#     train_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TrainSet\varden30SameMask'
#     train_entension_under = 'nii.gz'
#     test_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/test'
#     test_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TestSet\varden30SameMask'
#     test_entension_under = 'nii.gz'
#     val_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/AllSlices/1DVarden15Mask/val'
#     val_path_under = r'D:\Datasets\OASIS1\Undersampled-Registered\TestSet\varden30SameMask'
#     val_entension_under = 'nii.gz'
#     output_path = r'/raid/scratch/schatter/Output/Enterprise/v1-Gen3/IXI-T1/HH/AllSlices'
#     output_folder = 'Attempt8-IXIT1HHVarden1D15-Resnet2Dv214PReLU-SSIMLoss-AllSlices'
#     output_path = os.path.join(output_path, output_folder)
#     tb_logpath = os.path.join(tb_logroot, output_folder)
# else:
...
if IsGAN:
    ...
    model.CreateModel(netModel, IsCuda = IsCuda, IsMultiGPU = IsMultiGPU, n_channels=n_channels, 
                      initialLearningRate=initialLearningRate, lrDecayNEpoch=lrDecayNEpoch, lrDecayRate=lrDecayRate, betas=(beta1, beta2), epsilon=epsilon, loss_func=loss_func, IsImgKSPNet=IsImgKSPNet)
...
if(run_mode == 0):
    ...
    if val_path_fully is None: 
        ...
        train_set, _ = model.IntitializeMRITorchDS(dsClass, slicenoTrain, startingSelectSlice, endingSelectSlice, train_path_fully, train_path_under, train_entension_under, n_workers_train, getROIMode, mask)
        val_set, _ = model.IntitializeMRITorchDS(dsClass, slicenoTrain, startingSelectSlice, endingSelectSlice, val_path_fully, val_path_under, val_entension_under, n_workers_train, getROIMode, mask)
    ...
    if val_path_fully is None or val_path_under is None: 
        _, train_setDS = model.IntitializeMRITorchDS(dsClass, slicenoTrain, startingSelectSlice, endingSelectSlice, train_path_fully, train_path_under, train_entension_under, n_workers_train, getROIMode, mask) 
        model.TrainNValidate(dataset=train_setDS, output_path=output_path, total_n_epoch=n_epoch, start_epoch=start_epoch, num_workers=n_workers_train,valid_percent=valid_percent, save_frequency=save_frequency, save_best=save_best)
    else:
        print('Val set also supplied')
