...
class MRITorchDS(Dataset):
    ...
    def __init__(self, root_fully, root_under, fileExtension_under, domain='abs_fourier', transform=None, getROIMode=None, undersampling_mask=None, filename_filter = None, ds_split_xlsx = None, ds_split=None, do_sanity_check=False):
        """
            ...
            if (len(os.listdir(fullpath_subfolder_fully)) > 0):
                for file in files:
                    ...
                    if self.do_sanity_check:
                        vol_fully = FileRead3D(fullpath_file_fully)
                        vol_under = FileRead3D(fullpath_file_under)
                        ###some direct code, need to make it proper - just for testing
                        sanity = 1
                        if len(vol_fully.shape) != 4:
                            print('vol_fully not having shape len 4' + fullpath_file_fully)
                            sanity = 0
                        if len(vol_under.shape) != 4:
                            print('vol_under not having shape len 4' + fullpath_file_under)
                            sanity = 0
                        if vol_fully.shape != vol_under.shape:
                            print('vol_fully vol_under not having same len' + fullpath_file_fully)
                            sanity = 0
                        if not sanity:
                            continue
