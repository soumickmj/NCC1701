...
#under = sio.loadmat(r"E:\Datasets\MATs\20190320Fresh\OASIS-Subset1-Varden2D30SameMask-51to80SingleEach\ds1-image-train\0.mat")['under']
#ksp = sp.fft(under)

f = sio.loadmat(r"E:\Datasets\MATs\20190320Fresh\OASIS-Subset1-Varden2D05SameMask-51to80SingleEach\ds1-image-train\0.mat")['fully']
coord = sp.mri.radial(coord_shape=(30, 512, 2), img_shape=(256,256))
uK = sp.nufft(f, coord)
dcf = (coord[..., 0]**2 + coord[..., 1]**2)**0.5
u = sp.nufft_adjoint(uK*dcf, coord, oshape=(256,256))
#gridding = sp.gridding(uK, shape=(256,256), width=4.0, kernel, coord=coord)
#intrp = sp.interpolate(uK, width=4.0, kernel, coord=coord)

ksp = np.expand_dims(sio.loadmat(r"D:\ROugh\2d5.mat")['under'],0)

device = sp.Device(-1) #id = -1 represents CPU, and others represents the id_th GPUs.
if device.id > -1:
...
img_jsense_rss = np.sum(np.abs(img_jsense)**2, axis=0)**0.5
