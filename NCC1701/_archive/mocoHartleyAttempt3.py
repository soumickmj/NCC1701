...
#import pytorch_ssim
from utils.TorchLoss.MultiSSIM import MultiSSIM
...
#### Basic parameters
domain = 'hartley' #In which domain, network should work? Acceptable values: | fourier | hartley | image | compleximage |
n_coils = 1
...
filename_filter_train = (0,2000)
filename_filter_test = (3001,5000)
filename_filter_val = (2001,3000)
...
top_n_sampled_train = None
top_n_sampled_val = None
top_n_sampled_test = None
...
save_frequency = 1
IsResume = False
CheckpointPath = r"D:\Output\Gen3\ShepLog\MoCoHartley\Class2\Attempt1-MoCoHartleyShepLog-CInvUNet-SSIMLossInImg\checkpoints\checkpoint.pth.tar"
...
dsClass = importlib.import_module('utils.TorchDS.NPDS').NPDS
...
netModel = importlib.import_module('model.VAE.ConvVAE').ConvVAE 
NetType = 4 # 0: ordinary, 1: KSPNet4FastMRI, 2: Super Res Net, 3: KSPNet, 4: VAE
...
IsNegetiveLoss = True
loss_type = 1 #0: Calculate loss in the supplied domain. 1: bring data to image space before loss calculation, 2: same as 1 but normalized

netModel = netModel(n_channels=n_channels, batch_size=batch_size, no_of_sample = 1, starting_n_features=2, ZDIMS=20, input_size=(128,128), fc_features=1024, up_while_trans=False)

##FOR ALEX: TO USE MSE, uncomment the following two lines:-
#loss_func = nn.MSELoss
#IsNegetiveLoss = False

####TO USE SSIM, uncomment the following two lines
#loss_func = MultiSSIM()
#IsNegetiveLoss = True
...
if(pc_name == 'BMMR-Soumick'):
    ...
    train_path_fully = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_gt_hs.nii.gz"
    train_path_under = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_ma_hs.nii.gz"
    test_path_fully = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_gt_hs.nii.gz"
    test_path_under = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_ma_hs.nii.gz"
    val_path_fully = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_gt_hs.nii.gz"
    val_path_under = r"D:\Datasets\data-mc\data-motion-artifacts-removal\class-2\class_2_ma_hs.nii.gz"
    ds_split_xlsx_train = None
    ds_split_xlsx_test = None
    ds_split_xlsx_val = None
    output_path = r'D:\Output\Gen3\ShepLog\MoCoHartley\Class2'
    output_folder = 'Attempt4-MoCoHartleyShepLog-ConvVAE-GaussAndKLDLoss'
    ...
    train_path_fully = r'/nfs1/schatter/IXI/IXI-DATA-SuperResolution'
    test_path_fully = r'/nfs1/schatter/IXI/IXI-DATA-SuperResolution' 
    val_path_fully = r'/nfs1/schatter/IXI/IXI-DATA-SuperResolution'
    ds_split_xlsx_train = r"/nfs1/schatter/IXI/hh_split.xlsx"
    ds_split_xlsx_test = r"/nfs1/schatter/IXI/hh_split.xlsx"
    ds_split_xlsx_val = r"/nfs1/schatter/IXI/hh_split.xlsx"
    output_path = r'/nfs1/schatter/Output/Enterprise/v1-Gen3/IXI-T2-SuRes/HH/3D'
    output_folder = 'Attempt9-IXIT2SuResHH-interpolated-randomthick-Resnet3Dv2b14start32_4-MultiSSIMLoss-3D'
    output_path = os.path.join(output_path, output_folder)
    tb_logpath = os.path.join(tb_logroot, output_folder)
elif(pc_name == 'brain'):
    ##For GPU18 Cluster - For Undersampled Images
    tb_logroot = '/home/sciarra/NCC1701/TBLogs/Ent-Gen3'
    train_path_fully = r'/home/sciarra/NCC1701/IXI-DATA-SuperResolution'
    test_path_fully = r'/home/sciarra/NCC1701/IXI-DATA-SuperResolution' 
    val_path_fully = r'/home/sciarra/NCC1701/IXI-DATA-SuperResolution'
    ds_split_xlsx_train = r"/home/sciarra/NCC1701/hh_split.xlsx"
    ds_split_xlsx_test = r"/home/sciarra/NCC1701/hh_split.xlsx"
    ds_split_xlsx_val = r"/home/sciarra/NCC1701/hh_split.xlsx"
    output_path = r'/home/sciarra/NCC1701/Output/Enterprise/v1-Gen3/IXI-T2-SuRes/HH/3D'
    output_folder = 'Attempt9-IXIT2SuResHH-interpolated-randomthick-Resnet3Dv2b14start32_4-MultiSSIMLoss-3D'
    output_path = os.path.join(output_path, output_folder)
    tb_logpath = os.path.join(tb_logroot, output_folder)
else:
...
if(run_mode == 0):
    train_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, train_path_under, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, ds_split2use='train')
    ...
    if val_path_fully is None: 
        _, train_setDS = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, train_path_under, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, ds_split2use='train') 
        ...
        train_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, train_path_under, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, ds_split2use='train')
        val_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, val_path_fully, val_path_under, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_val, ds_split_xlsx_val, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, ds_split2use='val')
    ...
    test_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, test_path_fully, test_path_under, None, n_workers_test, getROIRes, undersampling_mat, filename_filter_test, ds_split_xlsx_test, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, ds_split2use='test')
