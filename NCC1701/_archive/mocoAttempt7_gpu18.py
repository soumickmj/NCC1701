...
from utils.MRITorchDSTransforms import CorruptByMotion, TransformFullyUpdateUnder, TransformAndUpdateFullyUnder, NormalizeTensor
from Math.FrequencyTransforms import fft2c
from Math.Normalizations import fnorm
from utils.fastMRI.TorchDSTransforms import to_tensor, complex_to_channel
...
trans_x = 0
trans_y = 0
transform_moco = TransformFullyUpdateUnder(CorruptByMotion(rotation, trans_x, trans_y))
#transform_ksp = TransformAndUpdateFullyUnder(transforms.Compose([fft2c, fnorm, to_tensor, complex_to_channel]))
transform_ksp = TransformAndUpdateFullyUnder(transforms.Compose([to_tensor, NormalizeTensor(type='minmax')]))
transform = transforms.Compose([transform_moco, transform_ksp])
...
save_frequency = 1
IsResume = True
CheckpointPath = r"/raid/scratch/schatter/Output/Enterprise/v1-Gen3/IXI-T1-MoCo/HH/Attempt8-IXIT1MoCoHH-RotMax25SliceWise-tx0ty0-Resnet2Dv2b14BS32-SSIMLoss-Slice60to90/checkpoints/checkpoint.pth.tar"
...
#from model.kSpace.CInvUNet import CInvUNet
#netModel = importlib.import_module('model.kSpace.EnsambleSpace').EnsambleSpace(n_channels, k_net=CInvUNet) 
netModel = importlib.import_module('model.ResNet.Resnet2Dv2b14').ResNet(do_batchnorm=True) 
NetType = 0 #0: Ordinary, 1: KSPNet, 2: SuperResNet
...
IsNegetiveLoss = True
loss_type = 0 #0: Calculate as it is. 1: Convert to Image Space and then calculate loss 2: 0+1 dual loss (not yet implimented) 
...
if(pc_name == 'BMMR-Soumick'):
    ...
    train_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\HH\Slice60to90\Varden1D30SameMask\train'
    test_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\HH\Slice60to90\Varden1D30SameMask\test' 
    val_path_fully = r'E:\Datasets\IXI\MATs\IXI-T1\HH\Slice60to90\Varden1D30SameMask\val'
    ds_split_xlsx_train = None
    ds_split_xlsx_test = None
    ds_split_xlsx_val = None
    output_path = r'D:\Output\Gen3\IXI-T1-MoCo\HH'
    output_folder = 'Attempt7-IXIT1MoCoHH-RotMax25-tx0ty0-Resnet2Dv2b14BS8-SSIMLoss-Slice60to90'
    ...
    train_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/Slice60to90/Varden1D30SameMask/train'
    test_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/Slice60to90/Varden1D30SameMask/test'
    val_path_fully = r'/nfs1/schatter/IXI/MATs/IXI-T1/HH/Slice60to90/Varden1D30SameMask/val'    
    ds_split_xlsx_train = None
    ds_split_xlsx_test = None
    ds_split_xlsx_val = None
    output_path = r'/raid/scratch/schatter/Output/Enterprise/v1-Gen3/IXI-T1-MoCo/HH'
    output_folder = 'Attempt8-IXIT1MoCoHH-RotMax25SliceWise-tx0ty0-Resnet2Dv2b14BS32-SSIMLoss-Slice60to90'
    output_path = os.path.join(output_path, output_folder)
    tb_logpath = os.path.join(tb_logroot, output_folder)
else:
...
if(run_mode == 0):
    train_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, None, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, transform=transform)
    ...
    if val_path_fully is None: 
        _, train_setDS = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, None, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, transform=transform) 
        ...
        train_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, train_path_fully, None, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_train, ds_split_xlsx_train, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_train, transform=transform)
        val_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, val_path_fully, None, None, n_workers_train, getROIRes, undersampling_mat, filename_filter_val, ds_split_xlsx_val, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_val, transform=transform)
    ...
    test_set, _ = model.IntitializeMRITorchDS(dsClass, None, None, None, test_path_fully, None, None, n_workers_test, getROIRes, undersampling_mat, filename_filter_test, ds_split_xlsx_test, corupt_type, mask_func, mask_seed, top_n_sampled=top_n_sampled_test, transform=transform)
...
AnalyzeResultFolder(output_path, '', False, False)
...
if(modelType_actual == '2DMultiSlice'):
    ...
    AnalyzeResultFolder(output_path+'-Combined', '', False, False)
