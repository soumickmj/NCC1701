...
from model.Wavelet.BasicWaveletNet import Image2ClassifyNet
...
if __name__ == '__main__':
    ...
    test_name = 'WaveletImage2ClassifyNet_Trainable_DO'
    num_epochs = 100
    ...
    model = Image2ClassifyNet(n_class=1,trainable_wavelets=True, do_drop=True)
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(val_loader):
            images = Variable(data['input'])
            labels = Variable(data['rotation'].to(device))
            outputs = model(images)
            loss = loss_fn(outputs, labels)
            loss = round(loss.data.item(),4)
            val_loss += loss
