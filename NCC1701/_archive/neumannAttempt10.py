...
###Resnet34+Disc2Dv2	2DSingleSlice	Yes, Cycle	1000	neumann	10	OASISRadial60	Radial
...
#dsClass = importlib.import_module('utils.TorchDS.AlexDS3_NoMask').MRITorchDS 
...
netModelD = importlib.import_module('model.GAN.Discriminator2Dv2').Discriminator 
...
if IsGAN:
    Model = importlib.import_module('model.ModelCycleGAN').Model 
else:   
...
initialLearningRate = 0.0001#0.0001# #0.0001 #0.1
beta1 = 0.9#0.9 #0.5 #0.9 #Momentum parameter of the gradients
beta2 = 0.999 #Momentum parameter of the gradients squared 
decay_start_epoch = 25 #For CycleGAN
...
if(pc_name == 'intelnuc03'):
    ...
    train_path_fully = r'D:\Datasets\AlexMultiCon3\Repro-DeepLearning\axial\Sorted-Vol1\PD\Train'
    train_path_under = r'D:\Datasets\AlexMultiCon3\Repro-DeepLearning\axial\Sorted-Vol1\T2\Train'
    train_entension_under = 'nii.gz'
    ...
    train_path_fully = '/scratch/tmp/schatter/Datasets/OASIS1/Radial/Train/full'
    train_path_under = '/scratch/tmp/schatter/Datasets/OASIS1/Radial/Train/spokes60'
    train_entension_under = 'nii'
    test_path_fully = '/scratch/tmp/schatter/Datasets/OASIS1/Radial/Test/full'
    test_path_under = '/scratch/tmp/schatter/Datasets/OASIS1/Radial/Test/spokes60'
    test_entension_under = 'nii'
    output_path = '/scratch/tmp/schatter/Output/Neural/EnterpriseV1/'
    output_folder = 'Attempt10-OASISRadial60-ResnetDiscV2Cycle1000-2DSingle'
    output_path = os.path.join(output_path, output_folder)
else:
...
model = Model(domain, batch_size, type = modelType, pin_memory = pinMemory, IsRadial = True)
if IsGAN:
    model.CreateModel(netModelG=netModel, netModelD=netModelD, IsCuda=IsCuda, IsMultiGPU=IsMultiGPU, n_channels=n_channels, n_epochs=n_epoch, decay_start_epoch = decay_start_epoch,
                      initialLearningRate=initialLearningRate, betas=(beta1, beta2))
else:
...
if(IsResume): #If IsResume is set to true, then resume from a given path or ask for a checkpoint path
    ...
    if IsGAN:
        model, start_epoch = model.helpOBJ.load_checkpointCycleGAN(checkpoint_file = CheckpointPath, model=model)
    else:
