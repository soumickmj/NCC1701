...
if __name__ == '__main__':
    ...
    test_name = 'ResNeXt101_PreT_BS20'
    checkpoint = None#r"D:\Datasets\Alex\T1_sag_1_for_Soumick\save\WaveletImageCorrectNClassifyNet_Trainable_DO_NOBranchSplit_Norm_BothMSELoss.pth.tar" #Make it None if you don't want to load
    runOnlyTest = False
    h5path4test = r'D:\Datasets\Alex\T1_sag_1_for_Soumick\ds.h5'
    testOutFile = r'D:\Datasets\Alex\T1_sag_1_for_Soumick\test.mat'
    num_epochs = 50
    loader_params = {'batch_size': 20, 'shuffle': False, 'num_workers': 0} #shuffle not needed as using SubsetRandomSampler
    ...
    if runOnlyTest:
        dataset = MoCoH5DS(h5path4test, device=device4reader, rotatenorm=True)
        ...
        for i, data in enumerate(test_loader):
            print(i)
            images = Variable(data['input']).cuda()
            ...
            outputImgs.append(img_output.cpu().detach().numpy())
        ...
        sio.savemat(testOutFile, {'OutLabels':outputClasses, 'OutImgs':outputImgs, 'Labels':inputLabels, 'InImgs': inputImgs, 'OriginalImgs': originalImgs})
    else:
        # Create Dataset, splits and data loader
        dataset = MoCoH5DS(h5path, device=device4reader, rotatenorm=True)
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(train_loader):
                images = Variable(data['input']).to(device)
                labels = Variable(data['rotation'].to(device)).unsqueeze(1)
                optimizer.zero_grad()
