...
modelType = '2D' #Type of Model, it can be 3D, 2DMultiSlice, 2DSingleSlice, 2D
IsGAN = False
...
trainMatFile = r'D:\Mat-DS\kspaceNUFFT_s60_train.mat'
testMatFile = r'D:\Mat-DS\kspaceNUFFT_s60_test.mat'

#### PyTorch Custom DS Class to use
dsClass = importlib.import_module('utils.TorchDS.MarioMAT').MRITorchDS #For DS following OASIS1 Standard
...
netModel = importlib.import_module('model.UNet.Unet2Dv2MarioV2').UNet 
...
if IsGAN:
    ...
    Model = importlib.import_module('model.ModelMAT').Model 
...
lrDecayNEpoch = 100 #4 #Decay the learning rate after every 4th epoch
...
lrDecayNEpochD = 5#4 #Decay the learning rate after every 4th epoch
...
if(pc_name == 'intelnuc03'):
    ...
    output_path = 'D:\Soumick\Output\MatMario1'
    output_folder = 'Attempt1-GAN-MarioUnet2-withBN'
    output_path = os.path.join(output_path, output_folder)
elif(pc_name == 'quadro'):
...
if(run_mode == 0):
    train_set, _ = model.IntitializeMRITorchDS(dsClass, trainMatFile, n_workers_train, getROIMode)
    ...
    _, train_setDS = model.IntitializeMRITorchDS(dsClass, trainMatFile, n_workers_train, getROIMode) #TODO , sliceno = slicenoTrain
    ...
    test_set, _ = model.IntitializeMRITorchDS(dsClass, testMatFile, n_workers_test, getROIMode)
