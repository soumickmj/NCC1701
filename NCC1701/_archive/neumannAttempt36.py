...
###Resnet2DF	2DSingleSlice	No	200	25		OASIS-varden30-samemask	undersampled			
...
mat_contents = sio.loadmat('/scratch/tmp/schatter/Code/EnterpriseV1/v3-20181020/1DVardenMask.mat')
...
slicenoTrain = 51 #Only for 2DSingleSlice
slicenoTrain = slicenoTrain - 1 #As array starts from 0
slicenoTest = 51 #Only for 2DSingleSlice
...
netModel = importlib.import_module('model.GAN.Resnet2DFReal').ResNet 
...
if IsGAN:
    ...
    Model = importlib.import_module('model.ModelF').Model 
...
if(pc_name == 'intelnuc03'):
    ...
    output_folder = 'Attempt0-ImageT1T2-Unet2Dv2-2DSingle'
    output_path = os.path.join(output_path, output_folder)
    ...
    output_folder = 'Attempt36_2-OASISVarden30SameMask-ResnetFReal-2SingleSlice51'
    output_path = os.path.join(output_path, output_folder)
else:
...
model = Model(mask, domain, batch_size, type = modelType, pin_memory = pinMemory)
if IsGAN:
