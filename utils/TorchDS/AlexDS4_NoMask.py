...
class MRITorchDS(Dataset):
    ...
    def __init__(self, root_fully, root_under, fileExtension_under, domain='image', transform=None, getROIMode=None):
        """
    ...
    def CreateImageBase(self, fullyContrast='PD', underContrast='T2', onlyBrain=True):
        ...
        for folder in subfolders_fully:
            subject = folder
            imagenameParts = subject.split('_')
            imagenameNoExtFully = imagenameParts[0] + '_' + fullyContrast + '_ON'
            imagenameNoExtUnder = imagenameParts[0] + '_' + underContrast + '_ON'
            
            if(onlyBrain):
                fullyImageName = imagenameNoExtFully+'_brain.'+self.fileExtension_fully #Creating file name for fully volume. By combining image name of fully without extension, dot, and extension of undersampled volumes
                fullpath_file_fully = os.path.join(self.root_fully, imagenameNoExtFully, fullyImageName)
                ...
                fullyImageName = imagenameNoExtFully+'.'+self.fileExtension_fully #Creating file name for fully volume. By combining image name of fully without extension, dot, and extension of undersampled volumes
                fullpath_file_fully = os.path.join(self.root_fully, imagenameNoExtFully, fullyImageName)
