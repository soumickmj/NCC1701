...
class MRITorchDS(Dataset):
    ...
    def __init__(self, root_fully, root_under, fileExtension_under, domain='abs_fourier', transform=None, getROIMode=None, undersampling_mask=None):
        """
            ...
            if (len(os.listdir(fullpath_subfolder_fully)) > 0):
                for file in files:
                    if (file.endswith('.hdr')): #.hdr files are ignored (as reading .img and .hdr for each volume is redundent)
                        continue
                    if('mpr-1' not in file): #To ignore redundant files in OASIS DS
                        continue
