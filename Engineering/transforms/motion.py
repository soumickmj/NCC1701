...
class Motion2Dv0(SuperTransformer):
    ...
    def apply(self, img):
        ...
        if self.n_threads > 1:
            ...
            pool.map(self.__perform_singlePE, range(
                self.aux.shape[1] if self.axis_selection == 0 else self.aux.shape[0]))
        else:
...
class Motion2Dv1(SuperTransformer):
    ...
    def __perform_singlePE(self, idx):
        img_aux = ndimage.rotate(
            self.img, self.random_rots[idx], reshape=False)
        img_h = np.fft.fft2(img_aux)
        if self.axis_selection == 0:
    ...
    def apply(self, img):
        ...
        if intext_ == 0:
            ...
            self.portion = np.sort(np.unique(np.random.randint(low=int(img.shape[dim]//2)-int(img.shape[dim]//n_+1),
                                                               high=int(
                                                                   img.shape[dim]//2)+int(img.shape[dim]//n_+1),
