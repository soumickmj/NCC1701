...
from utils.CS import Undersamp, CSReco, Hendrik_PROSIT
from support.ApplyDataConsistancy import MinMaxNorm
...
class Model(object):
    ...
    def __init__(self, domain, batchSize = 1, imageSize = None, type = '2DMultislice', pin_memory = False, IsRadial = False, undersamplingMask = None, log_path = None, maskORom_path=None, CSMode=0):
        ...
        self.maskORom_path = maskORom_path
        self.CSMode = CSMode
    ...
    def CreateModel(self, netModel, IsCuda = False, IsMultiGPU = False, n_channels = 1, initialLearningRate = 0.001, lrDecayNEpoch = 4, lrDecayRate = 0.5, betas = (0.9, 0.999), epsilon = 1e-08, loss_func = nn.MSELoss, optimizer=optim.Adam):
    ...
    def Validate(self, dataloader, saveOutput=False, print_freq=10, epoch=None, n_epoch=None, output_path=None, IsTest=False):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset (testing/prediction).
                ...
                # under_ksp, under_img, mps = Hendrik_PROSIT(fully_img=fully.squeeze().numpy(), maskORom_path=self.maskORom_path)
                under_ksp, under_img, mps = Undersamp(fully_img=fully.squeeze().numpy(), maskORom_path=self.maskORom_path)
                generated = MinMaxNorm(abs(CSReco(under_ksp, mode=self.CSMode)))
                generated = torch.from_numpy(generated).unsqueeze(0).unsqueeze(0).float()
