...
"""
Architecture of Unet 2D, converted to 3D
Uses 3D Images
...
class UNet_down_block(torch.nn.Module):
    def __init__(self, input_channel, output_channel, kernel_size=3, stride=1, padding=1, pool_size=2, pool_stride=2, down_size=True, do_batchnorm=True):
        ...
        self.conv1 = torch.nn.Conv3d(input_channel, output_channel, kernel_size,  stride=stride, padding=padding)
        self.bn1 = torch.nn.BatchNorm3d(output_channel)
        self.conv2 = torch.nn.Conv3d(output_channel, output_channel, kernel_size, stride=stride, padding=padding)
        self.bn2 = torch.nn.BatchNorm3d(output_channel)
        self.conv3 = torch.nn.Conv3d(output_channel, output_channel, kernel_size, stride=stride, padding=padding)
        self.bn3 = torch.nn.BatchNorm3d(output_channel)
        self.max_pool = torch.nn.MaxPool3d(pool_size, pool_stride)
...
class UNet_up_block(torch.nn.Module):
    def __init__(self, prev_channel, input_channel, output_channel, scale_factor=2, upsample_mode='nearest', kernel_size=3, stride=1, padding=1, do_batchnorm=True):
        ...
        self.conv1 = torch.nn.Conv3d(prev_channel + input_channel, output_channel, kernel_size, stride=stride, padding=padding)
        self.bn1 = torch.nn.BatchNorm3d(output_channel)
        self.conv2 = torch.nn.Conv3d(output_channel, output_channel, kernel_size, stride=stride, padding=padding)
        self.bn2 = torch.nn.BatchNorm3d(output_channel)
        self.conv3 = torch.nn.Conv3d(output_channel, output_channel, kernel_size, stride=stride, padding=padding)
...
class UNet(torch.nn.Module):
    def __init__(self, n_channels, do_batchnorm=True):
        ...
        self.last_conv2 = torch.nn.Conv3d(16, n_channels, 1, padding=0)
        self.relu = torch.nn.ReLU()
